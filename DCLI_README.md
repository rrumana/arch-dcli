...
> **⚠️ BETA SOFTWARE** - Use at your own risk. Always maintain backups before major operations.
...
### Package Management
- **Declarative Configuration** - Define packages in YAML or Lua, sync your system to match
...
If AUR installs fail, clone the repo and run the installer directly (Lua is vendored; no system Lua required):
...
This builds from source with the same script used in the AUR package. Requires a Rust toolchain; all features (including Lua scripting) are enabled by default.
...
This produces `target/release/dcli` with Lua scripting compiled in (vendored Lua 5.4).
...
**Current limitations (Lua configs):**
- `dcli module enable/disable` (including interactive) cannot edit `.lua` host/config files; update `enabled_modules` manually or use YAML if you want automatic edits.
- `dcli service enable/disable` cannot write to `.lua` configs; update `enabled_service_profiles` manually or switch to YAML.
- `dcli edit` does not list `.lua` configs; open and edit your Lua files directly.
- Coming soon: safe write support for Lua configs so these commands can update them automatically.

**Both YAML and Lua work together:**
- Modules can be YAML or Lua (mix and match)
- Host files can import both formats
- Use YAML for simple configs, Lua for complex logic
